# ==========================================================
# Tennessee Eastman Adaptive Digital Twin
...
class CUSUMDetector:
    ...
    def update(self, error):
        ...
        self.S = max(0, self.S + error - self.k)
...
def fine_tune_model(model, buffer, device):
    ...
    for _ in range(ADAPT_EPOCHS):
        for xb, residual, last_state in buffer:
            xb = xb.to(device)
    ...
    print("Adaptive update completed\n")
...
def main():
    ...
    with torch.no_grad():
        for xb, residual, last_state in val_loader:
            ...
            batch_pred = pred_next.cpu().numpy()
            batch_true = true_next.cpu().numpy()

            preds.append(batch_pred)
            actuals.append(batch_true)

            batch_error = np.mean(np.abs(batch_true - batch_pred))

            residual_monitor.update(batch_error)

            drift = drift_detector.update(batch_error)

            adaptation_buffer.append((xb, residual, last_state))

            if len(adaptation_buffer) > ADAPT_BUFFER_SIZE:
                adaptation_buffer.pop(0)

            if drift:
                fine_tune_model(model, adaptation_buffer, DEVICE)
